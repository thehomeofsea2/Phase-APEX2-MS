...
# Step 2: Wait for user to fill in
...
cat("  Contains: Module01 all data + data_raw + sampleGroup\n")
...
annotation_mode <- "Nucleolus"

# Advanced: If custom TP annotation columns are needed, provide a list here (will override annotation_mode if set)
...
cat("  Contains: Module01-02 all data + data_annotated + annotation_references\n")
...
cat("  Contains: Module01-03 all data + standardized_data_list + norm_types_used\n")
...
cat("  Contains: Module01-04 all data + imputed_data_list + imputation_params\n")
...
# If you want to use exclusion rule examples:
# correlation_config$exclude_context <- c("Control")  # Only keep Experiment groups
# correlation_config$exclude_pltype <- c("PL")        # Exclude PL groups

# If you want to directly specify bioGroup examples:
...
cat("  Contains: Module01-05 all data + heatmap_info\n")
...
# Module 7 will automatically build comparison groups based on sampleGroup$FirstROCgroup
# FirstROCgroup logic:
...
cat("  Contains: Module01-06 all data + diff_results1, comparisons_used, comparison_info\n")
...
roc_annotation_column <- "Nucleolus_main_Localization"  # Annotation column for ROC analysis
tp_label <- "Nucleolus"                            # True Positive label
fp_label <- "Mitochondrion"                         # False Positive label (after SubMito conversion)
enable_submito <- FALSE                       # Enable SubMito conversion
...
cat("  Contains: Module01-07 all data + roc_results1, roc_thresholds1, data_with_submito, expr_fdr_df_list\n")
...
# Automatically generates Method A and Method B results:
...
# no_fdr_comparisons: Vector of comparisons without FDR filtering in Method A (default NULL, i.e., process as Method B)
# use_roc_threshold: Whether to use ROC threshold (TRUE) or fixed FC threshold (FALSE)
# fixed_fc_threshold: Fixed FC threshold if not using ROC threshold
# min_valid_lfq: Minimum number of valid LFQ values (default 2, for catalytic group filtering)
...
fdr_threshold <- 0.05
# Method A: Specify comparison vector without FDR filtering (if NULL, Method A equals Method B)
no_fdr_comparisons <- c("E7A2B4_Light_vs_A2B4_Light","E7C2_Light_vs_C2_Light")  # Can be set to NULL to use default Method B
use_roc_threshold <- TRUE
fixed_fc_threshold <- NULL  # If use_roc_threshold=FALSE, set fixed threshold such as 2.0
min_valid_lfq <- 2
annotation_column <- "Nucleolus_main_Localization"
...
cat("  Contains: Module01-08 all data + filtered_data_A, filtered_data_B, merged_data_A, merged_data_B\n")
...
cat("  Contains: Module01-09 all data + replaced_data_A, replaced_data_B\n")
...
cat("  Contains: Module01-10 all data + FDR_combined_df_list_2nd, etc.\n")
...
second_roc_annotation_column <- "Nucleolus_main_Localization"
second_roc_tp_label <- "Nucleolus"
second_roc_fp_label <- "Nuclear"
...
cat("  Contains: Module01-11 all data + second ROC results\n")
...
subsg_mode <- "subNucleolus"  # Optional: "subSG" or "subNucleolus"
...
cat("  Contains: Module01-12 all data + Expr_FDR_df_list_2nd_SubSGs\n")
...
cat("✓ Detected Expr_FDR_df_list_2nd_SubSGs, can continue configuring Module 14 volcano plot parameters\n")
...
if (is.null(module14_comparison_metadata)) {
  cat("⚠ Warning: Could not obtain comparison classification information (comparisons_list_2nd or bioGroup_info_2nd missing)\n")
} else {
  cat("✓ Built comparison classification based on Module11 information (Exp_vs_Exp / Exp_vs_Spatial)\n")
}
...
# - volcano_use_threshold_colors: TRUE means color by threshold, ignore annotation colors
...
# - volcano_label_annotations: Which annotation categories allow label display
# - volcano_comparison_sets: Define logFC & FDR column combinations
volcano_versions <- c("Local_QNorm_New_B") # Optional: names(Expr_FDR_df_list_2nd_SubSGs), NULL=all
volcano_annotation_column <- "Nucleolus_Localization" # Optional: *_Localization columns printed above
volcano_annotation_color_map <- c(
  "Nucleolus" = "red",
  "Mitochondrion" = "blue"
) # Unlisted annotations will automatically fall back to grey
...
volcano_label_mode <- c("without") # Allow "with" or "without", can fill both
volcano_label_annotations <- c("Nucleolus","Mitochondrion") # Only these annotations allow labels
volcano_label_size <- 3
volcano_label_max_overlaps <- 15
# Optional: c("Exp_vs_Exp","Exp_vs_Spatial"), or select one category, NULL=all
volcano_comparison_categories <- c("Exp_vs_Spatial")
volcano_xlim_override <- c(-3, 4)
volcano_ylim_override <- c(0, 7.5)
# Custom example (if you need to replace default values):
...
cat("  Contains: Module01-13 all data + volcano plot PDFs\n")
...
if (!requireNamespace("openxlsx", quietly = TRUE)) {
  warning("⚠ Warning: Missing openxlsx package, cannot export ForStep16 Excel, please install openxlsx first")
} else {
