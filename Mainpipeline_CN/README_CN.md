...
# 2. 主流程
按照guide.md说明，在R(R 4.3.3)中运行main_pipeline_SG.R
...
# 3. 1D/2D模型构建与PPI分析
使用main_pipeline.R输出的模块14.RData作为Step29_BaseModels.R的输入，运行251117_BaseModels_Concise.R脚本。脚本中为模版，需要根据使用情况更改各种接口。脚本依赖从Module14.RData中继承的ForStep16列表，进入Step29 多模型复杂需求分析部分时需要将ForStep16重命名为ForStep19对象.

# 4. 模型性能比较
使用main_pipeline.R输出的模块14.RData作为Step29_ModelComparision.R的输入，同样需要改接口，但是保留最后的ForStep16对象，进入后续计算时需要将ForStep16重命名为ForStep19对象
