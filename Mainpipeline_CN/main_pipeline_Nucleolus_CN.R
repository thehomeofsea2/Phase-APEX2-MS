...
# ROC分析参数
roc_annotation_column <- "Nucleolus_main_Localization"  # 用于ROC分析的注释列
tp_label <- "Nucleolus"                            # True Positive标签
fp_label <- "Mitochondrion"                         # False Positive标签（SubMito转化后）
enable_submito <- FALSE                       # 启用SubMito转化
...
no_fdr_comparisons <- c("E7A2B4_Light_vs_A2B4_Light","E7C2_Light_vs_C2_Light")  # 可设为NULL使用默认Method B
...
subsg_mode <- "subNucleolus"  # 可选："subSG" 或 "subNucleolus"
...
volcano_versions <- c("Local_QNorm_New_B") # 可选：names(Expr_FDR_df_list_2nd_SubSGs)，NULL=全部
volcano_annotation_column <- "Nucleolus_Localization" # 可选：上方打印的 *_Localization 列
...
volcano_label_annotations <- c("Nucleolus","Mitochondrion") # 仅这些注释允许打标签
