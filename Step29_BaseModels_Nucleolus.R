# Military Academy SG sample analysis
...
##Replace with any environment that can complete the main pipeline up to the FC/Scatter plot
...
# Define myTP_vector2 for Fig9 TPR curve - include all categories containing "TP"
...
# Spatial control group interface: configure each control group by matching column name patterns and display names
spatial_control_groups <- list(
  J37 = list(
    pattern = "J37_H2O2_LFQ",          # Column name matching pattern (regex) for finding corresponding LFQ columns
    display_name = "J37 (H2O2-NLS)"    # Display name in plots and outputs
  ),
  K19 = list(
...
# MULTIBAIT_LEVEL_ORDER: control ggplot stacking order (first element is stacked at the bottom) to align schematics
...
# MULTIBAIT_SG_CATEGORIES: specify which MultiBait categories to label, filtering labels by this set
...
# Quantitative column non-NA threshold interface
#    - PARAM_MIN_LFQ_NONNA: minimum number of non-NA LFQ values required for the Phase3 and MultiBait paths before process_replicates()
#      ensures each group's LFQ column has at least this many non-NA values (default keeps 1)
#    - PARAM_MIN_LFQ_NONNA_TPR: dedicated threshold for ranking Fig9 TPR candidates to control the minimum non-NA count before TP/FP calculations
#      minimum required non-NA count (default 2) to keep the curve based on stable replicates
PARAM_MIN_LFQ_NONNA <- 2
PARAM_MIN_LFQ_NONNA_TPR <- 2



#Step29 Multi-model complexity requirements (version)#####
...
# Due to repeated parallel failures, switch to sequential processing for stability
...
# === Step 1: load the STRING alias mapping table (one-time) ====
...
# Filter aliases sourced from gene names (keep multiple sources to improve mapping rate)
...
# === Step 2: load the STRING interaction network (one-time) ====
...
# STRING files are space-delimited, so specify the separator
string_links <- read.table(LOCAL_STRING_LINKS_FILE, 
                           header = TRUE,           # First row is column names
                           sep = " ",               # Space-separated
                           stringsAsFactors = FALSE,
                           comment.char = "",       # Do not process comments
                           quote = "")              # Do not process quotes

# Print the actual column names for debugging
cat(paste("  Actual column names:", paste(colnames(string_links)[1:5], "..."), "\n"))
cat(paste("  Total", ncol(string_links), "columns,", format(nrow(string_links), big.mark=","), "rows\n"))

# Auto-detect column names (STRING may use different formats)
col_names <- colnames(string_links)

# Identify the first protein column (usually column 1)
protein1_col <- col_names[1]
# Identify the second protein column (usually column 2)
protein2_col <- col_names[2]
# Identify the combined_score column (might be named combined_score or similar)
score_cols <- col_names[grepl("combined", col_names, ignore.case = TRUE)]
if (length(score_cols) == 0) {
  # If no combined column is present, try to find score columns
  score_cols <- col_names[grepl("score", col_names, ignore.case = TRUE)]
  if (length(score_cols) == 0) {
    stop("Error: score column not found in the file")
  }
}
...
for(df_name in names(ForStep19)) {
  cat(paste("Processing PPI data source:", df_name, "\n"))
  ...
  tryCatch({
    ...
    if (nrow(mapped_proteins) == 0) {
      cat("  ⚠️  Warning: no proteins mapped to STRING IDs; skipping this data source\n")
    ...
    # Filter interactions to keep only mapped proteins
    ...
    cat(paste("  - Retrieved", format(nrow(ppi_data$interactions[[df_name]]), big.mark=","), 
              "interaction pairs\n"))
    
    # Calculate global PPI scores (total interaction strength per protein)
    ...
    cat(paste("  - Computed global PPI scores for", nrow(ppi_data$ppi_global[[df_name]]), 
              "proteins\n"))
    
  }, error = my_error_handler)
}

cat("✅ Local PPI preprocessing complete.\n\n")
...
cat("⚠️  PPI data is empty; creating empty PPI features\n")
  ...
  if(nrow(interactions_df) > 0) {
    ...
    # Set to 0 if there is no PPI data
  ...
  tryCatch({
    # Reload necessary libraries inside each parallel worker
    library(dplyr)
    library(pROC)
    ...
    cat(paste("Testing", length(models_to_run), "base models\n"))
    ...
    for(model_name in names(models_to_run)) {
      ...
      tryCatch({
        ...
        if(model_info$type == "1D") {
          ...
          cat("    ✅ 1D model trained successfully\n")
        } else { # GLM for 2D/3D
...
# -------------------------------------------------------------------
cat("Starting parallel analysis of all combinations...\n")
...
cat("\nAll serial computation tasks completed.\n")


# -------------------------------------------------------------------
# 6. Consolidate Results and Export Files
# -------------------------------------------------------------------
cat("Summarizing results and exporting files...\n")
...
for(i in seq_along(all_results)) {
  ...
  if (is.null(result) || !is.list(result)) {
    ...
    warning(paste("Task", failed_task$df_name, "-", failed_task$group, "failed or returned invalid results; skipped."))
    next
  }
...
cat(paste("\nModel performance summary saved to:", file.path(output_dir, "full_model_comparison_summary.csv"), "\n"))
...
if(length(final_candidates_list) > 0) {
  ...
  cat(paste("Protein scoring details saved to:", file.path(output_dir, "all_proteins_scored_summary.csv"), "\n"))
}
...
cat(paste("R object for second-phase plotting saved to:", file.path(output_dir, "analysis_phase1_output.RData"), "\n"))
...
plan(sequential)
cat("\nParallel session terminated. Script complete.\n")
...
cat("Loading Phase 1 results 'analysis_phase1_output.RData'...\n")
...
# -------------------------------------------------------------------
# 2.Global Plotting Configuration (modify the appearance of all plots here) ####
# -------------------------------------------------------------------
...
# 6. Number of density map clusters (set to 0 to disable clustering)
density_kmeans_k <- 0


# --- NEW: Data-driven Axis Range Interfaces ---
# Note: set to a vector like c(-5, 5) to override the range manually; leave as NULL to use the automatically computed global range.
...
# Coordinate mode: "rank01" uses normalized rank coordinates [0,1] with fixed bandwidth; "rank" keeps the original 1..n rank range with the default bandwidth
density_coord_mode <- "rank01"



# Legacy code interfaces; do not remove them to avoid dependency errors even though they are not used in calculations
#-----
...
#-------------------------------------------------------------------------------------
#----Configurable region---------------------------------------------------------Configurable region
tp_weight_both <- 2
tp_weight_single <- 1.0
tp_weight_none <- 0.25

# Override coordinate mode according to the master switch
density_coord_mode <- if (use_normalized_density) "rank01" else "rank"


# JP: joint ranking probability heatmap (unweighted)
jp_enable <- TRUE
jp_pair_mode <- "directed"   # "directed"|"undirected" (currently directed)
jp_grid_mode <- "fixed"
jp_grid_bins_fixed <- 50L
# Optional: lower percentile (recommended 0~0.1); set to 0 to keep the background unchanged
...
cat("Precomputing unified axes and color ranges for all plots...\n")
...
cat("Precomputing global maximum density for density plots...\n")
...
for (res in all_results) {
  ...
  # Filter and symmetrize interactions (consistent with the plotting function)
  ...
  # Use the same column names as the plotting function for consistency
  ...
  if (nrow(coords) > 10) {
    tryCatch({
      # Note: coordinate order must match the plotting function (coords[,2], coords[,1])
      # Tile size #####
...
cat(paste("All Fig6 density plots will use a unified max density of:", round(global_max_density, 8), "\n"))
...
if (identical(tolower(density_coord_mode), "rank01") || density_max_quantile < 1 || density_max_quantile > 1) {
  ...
  if (length(density_max_vals) > 0) {
    ...
    cat(paste("Fig6 global max density replaced with normalized/quantile:", round(plot_scales_config$max_density, 8),
              paste0(" [q=", density_max_quantile, "]\n"), sep = ""))
  }
}

# Precompute the global maximum density for candidate protein density plots (Fig8)
cat("Precomputing global max density for candidate protein density plots (Fig8)...\n")
...
for (res in all_results) {
  ...
  # Loop through all models in this result
  for (model_name in names(res$candidates_full_list)) {
    tryCatch({
      # Get candidate proteins that pass the threshold
...
cat(paste("All Fig8 candidate density plots will use a unified max density of:", round(global_max_density_candidates, 8), "\n\n"))

# ================== Weighted density global maxima (separate) ==================
cat("Precomputing weighted density (Fig6) global max density/quantiles...\n")
...
for (res in all_results) {
  ...
  # Take weights (combined_score), ensuring they are non-negative and non-NA
...
cat(paste("Weighted Fig6 density plots will use a unified max density quantile",
          paste0(" [q=", weighted_density_max_quantile, "]:"),
          round(plot_scales_config$max_density_weighted, 8), "\n"))

cat("Precomputing weighted candidate density (Fig8) global max density/quantiles...\n")
...
cat(paste("Weighted Fig8 candidate density plots will use a unified max density quantile",
...
if (identical(tolower(density_coord_mode), "rank01") || density_candidates_max_quantile < 1 || density_candidates_max_quantile > 1) {
  ...
  if (length(density_max_vals_candidates) > 0) {
    ...
    cat(paste("Fig8 candidate density cap replaced with normalized/quantile:", round(plot_scales_config$max_density_candidates, 8),
              paste0(" [q=", density_candidates_max_quantile, "]\n\n"), sep = ""))
  }
}


# ================== PM/JP global color upper-bound precomputation ==================
cat("Precomputing PM/JP global color upper bounds...\n")

# Utility function: retrieve TP/Other grouping column names
...
# PM weight function (reads global tp_weight_* and combined_score fields)
...
# Compute the maximum cell probability in PM (all proteins)
...
# Compute the maximum cell probability in PM (candidates, taking the maximum across models)
...
# Compute the maximum cell probability in JP (all, unweighted, directed)
.jpMaxForResult <- function(res, bins) {
  ...
  # Bin protein counts along the axis (directed)
  genes_idx <- 1:n_genes
...
# Compute the maximum cell probability in JP (candidates, unweighted)
...
# PM: calculate upper quantiles for all/candidates under different weight modes
...
cat("PM global upper quantiles computed:\n"); print(pm_max)
cat("PM candidate upper quantiles computed:\n"); print(pm_cand_max)

# PM: minimum cell probability quantile for color floors, based on non-zero bins
...
cat(paste("JP overall max quantile:", round(plot_scales_config$jp_max_prob, 8), "\n"))
cat(paste("JP candidate max quantile:", round(plot_scales_config$jp_candidates_max_prob, 8), "\n"))

# JP: minimum cell probability quantile (based on non-zero bins)
...
# ================== JP Weighted global color bounds precomputation ==================
cat("Precomputing JP weighted global color bounds...\n")
...
getJpNormalizedWeights <- function(symmetric_interactions, node_attr, weight_mode) {
  ...
  if (identical(weight_mode, "combined_score")) {
    ...
    # Normalize to <=1 by dividing the product by the max TP weight (equivalent to base_w * (tp_w_raw/max_tp_w))
    w <- base_w * tp_w
  }
  # Power boosting
...
# Compute weighted JP upper/lower bounds for all proteins and candidates
...
plot_model_performance <- function(result, plot_dir_roc, plot_dir_youden, cfg) {
  ...
  if (length(roc_plots) > 0) {
    ...
    cat(paste("  -> ROC and Youden plot data exported:", df_name, "-", group_name, "\n"))
  }
}
...
plot_3d_scatter <- function(result, scales_config, cfg, plot_dir) {
  ...
  # Retrieve decision boundary thresholds
  thresholds <- sapply(result$candidates_full_list, function(x) x$Model_best_threshold[1])
  
  # Use the configured axis ranges to align with the decision boundaries
  get_lim <- function(override, fallback) { if (!is.null(override)) override else fallback }
  ...
  # Compute decision boundary parameters for 2D models
  ...
  # Draw the first plot: colored by global PPI
  ...
  # Draw the second plot: colored by local PPI
...
plot_ppi_heatmap <- function(result, cfg, plot_dir){
  ...
  # Filter interactions for the current group
  filtered_interactions <- interactions_df %>% 
    dplyr::filter(protein1 %in% sorted_genes & protein2 %in% sorted_genes)
  ...
  # Symmetrize interactions: add (B,A) for each (A,B) so the pair matrix stays symmetric along the diagonal
  symmetric_interactions <- bind_rows(
    filtered_interactions,
    filtered_interactions %>% 
      rename(protein1 = protein2, protein2 = protein1, combined_score = combined_score)
  ) %>%
  distinct(protein1, protein2, .keep_all = TRUE)  # Remove duplicates while keeping the first occurrence's score
  
  plot_df <- symmetric_interactions %>%
    mutate(x = gene_map[protein1], y = gene_map[protein2], score = combined_score) %>%
    dplyr::select(x, y, score)
  
  # Export the plotting data matrix for inspection
  ...
  cat(paste("  -> PPI Heatmap plot data exported:", output_csv_path, "\n"))
  ...
  # Support both plot_group and group_Localization column names
...
# 4f. Candidate Proteins PPI Heatmaps (based on proteins passing the threshold) ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
plot_candidate_ppi_heatmaps <- function(result, cfg, plot_dir){
  ...
  cat(paste("  Generating candidate protein PPI heatmap for", df_name, "-", group_name, "...\n"))
  
  # Generate heatmaps for each model
  for (model_name in names(result$candidates_full_list)) {
    tryCatch({
      # Get the candidate proteins passing the threshold for that model
      ...
      if (length(pass_proteins) < 5) {
        cat(paste("    ⚠️  Model", model_name, "has too few candidates (<5); skipping heatmap\n"))
        next
      }
      ...
      if (nrow(filtered_interactions) == 0) {
        cat(paste("    ⚠️  No interactions between candidate proteins; skipping\n"))
        next
      }
      ...
      group_col <- if ("plot_group" %in% colnames(sorted_candidates)) {
        ...
        cat("    ⚠️  Group column not found; skipping annotation bar\n")
        NULL
      }
      ...
      if (!is.null(group_col)) {
        ...
        # Use blanks when no grouping column exists
        p_top <- plot_spacer()
        p_right <- plot_spacer()
      }
...
# 4h. Grid tools and core matrix calculations ####

#' @title Compute the PM matrix and lower-left cumulative
#' @description 2D probability mass (PM) matrix: bin interactions by normalized rank coordinates, sum bin weights to obtain probability mass, and compute cumulative sums in the lower-left corner across thresholds.
#' @param node_attr Node attribute data frame (includes Gene, mean_group_medianNorm, etc.)
#' @param interactions Interaction data frame (protein1, protein2, combined_score)
#' @param bins Integer number of grid bins (recommend 100)
...
#' @param candidate_genes Optional character vector to restrict to candidate subsets
#' @return List: matrix_df (long form with x_bin, y_bin, prob), summary_df (lower-left cumulative), bins
computePmMatrixWithSummary <- function(node_attr, interactions, bins, cfg, weight_mode = "none", candidate_genes = NULL) {
  ...
  df <- data.frame(x_bin = xb, y_bin = yb, w = w)
  agg <- df %>% group_by(y_bin, x_bin) %>% summarise(v = sum(w), .groups = "drop")
  # Fill the entire grid, marking missing bins as 0 to keep the background populated
...
#' @title Compute the JP matrix and lower-left cumulative (unweighted, directed)
#' @description Compute a probability matrix of observed vs possible PPI logs per bin and report its lower-left cumulative sums.
#' @param node_attr Node attributes
#' @param interactions Interaction data
#' @param bins Number of bins
#' @param candidate_genes Optional candidate subset
#' @return List: matrix_df (long form with x_bin,y_bin,probability), summary_df (lower-left cumulative)
...
#' @title Compute the weighted JP matrix and lower-left cumulative (directed)
#' @description Numerator = weighted sum within bins (normalized and power-transformed weights); denominator = possible log (nx*ny); output probability matrix and lower-left cumulative sums.
#' @param node_attr Node attributes
#' @param interactions Interaction data
#' @param bins Number of bins
...
# 4i. PM/JP plotting functions (all proteins) ####

#' @title JP heatmap (all proteins, unweighted, directed)
plotJointProbHeatmap <- function(result, scales_config, cfg, plot_dir) {
  ...
  p <- ggplot(mat_df, aes(x = x_bin, y = y_bin)) +
    ...
    labs(title = paste(df_name, "-", group_name), subtitle = "Joint Probability Heatmap (directed)",
         x = "Column bin", y = "Row bin")
  pdf_path <- file.path(plot_dir, paste0(df_name, "_", group_name, "_JP.pdf"))
  ggsave(pdf_path, p, width = 9, height = 8)
}

# 4l. JP weighted heatmap (all proteins) ####

#' @title JP weighted heatmap (all proteins, directed)
...
for (i in seq_along(all_results)) {
  ...
  tryCatch({
    ...
    # 15: JP all proteins (weighted, multi-mode)
...
# ===================================================================
#
### Phase 3: Candidate protein selection, summarization, and visualization comparison (v3.0 - advanced configuration)
#
# ===================================================================
...
# Ensure the required R packages are loaded
...
# Define the functional error handler
my_error_handler <- function(e) {
  cat("Error occurred:", conditionMessage(e), "\n")
  return(NULL) # Return NULL for downstream handling
}

# --- Load Phase 1 analysis results ---
tryCatch({
  load("Step29_Analysis_Phase1_DataExport/analysis_phase1_output.RData")
  cat("Phase 1 analysis result 'analysis_phase1_output.RData' loaded successfully.\n")
}, error = function(e) {
  stop("Failed to load the Phase 1 RData file. Please ensure the path 'Step29_Analysis_Phase1_DataExport/analysis_phase1_output.RData' is correct.")
})
...
# 2. Choose models to show in the figures
#    Retrieve all model names from all_results[[1]]$auc_scores
#    "logFC>0.5 & FDR<0.05" and "Control" are reserved for traditional methods and controls
...
# Comment out any models that are not needed
...
# 3. Identify columns used for grouping and color fills
#    Options: "MultiBait_Localization", "HaloMap_Localization", "GO_Localization", "group_Localization"

PARAM_GROUPING_COLUMN <- "group_Localization"

# 4. Choose the categories to annotate on the plots (must be one of PARAM_GROUPING_COLUMN's values)
PARAM_ANNOTATION_CATEGORY <- "TP"

# 5. [User review/update required] Define the color scheme
#    Note: this will be checked in the next step; after viewing the script output, make sure the names here match the categories exactly
PARAM_LOCALIZATION_COLORS <- c(
  "TP" = "#e41a1c",
  "Other" = "#bdbdbd"
  # Example: if using MultiBait_Localization, provide a complete color scheme as shown here
...
# ===================================================================
#
### Section B: Data preprocessing and configuration checks
#
# ===================================================================


# --- 1. Data preprocessing (logical optimizations) ---
# a. Add a group_Localization column to each all_results object
for (i in seq_along(all_results)) {
...
cat("Preprocessing 1/2 complete: 'group_Localization' column added to 'all_results'.\n")

# b. Also add group_Localization columns to each data frame inside ForStep19
...
cat("Preprocessing 2/2 complete: 'group_Localization' column added to all data frames in 'ForStep19'.\n")
names(ForStep19$noMBR_Local_QNorm_New_B)
unique(ForStep19$noMBR_Local_QNorm_New_B$group_Localization)


# --- 2. Dynamically verify the color configuration ---
...
cat(paste("Detected the following categories in grouping column '", PARAM_GROUPING_COLUMN, "':\n", sep=""))
...
if (!all(detected_levels %in% defined_colors)) {
  ...
  stop(paste("Color configuration error! The following categories exist in the data but do not have colors defined in 'PARAM_LOCALIZATION_COLORS':\n",
             paste(missing_levels, collapse = ", ")))
}
if (!all(defined_colors %in% detected_levels)) {
  extra_colors <- setdiff(defined_colors, detected_levels)
  warning(paste("Color configuration warning: The following colors are defined but not present in the data categories:\n",
...
# ===================================================================
#
### Section C: Core functionality and analysis loops
#
# ===================================================================
#### Added a helper function process_replicates for convenience ####

#' @title Perform QC and aggregate replicate samples by group
#' @description Automatically QC the specified columns to ensure they belong to the same replicate group before computing aggregated summaries.
#' @param data A data frame.
#' @param group_col Column name used for grouping (bare names or strings).
#' @param RepRange Numeric range indicating which quantitative columns to aggregate, e.g., `2:4`.
#'
#' @return Returns a new data frame with aggregated values if QC passes; otherwise raises an error.

process_replicates <- function(data, group_col, RepRange) {
  # --- 1. QC step ---
  # Check that RepRange does not exceed the data frame bounds
  if (max(RepRange) > ncol(data) || min(RepRange) < 1) {
    stop("QC error: RepRange exceeds the number of columns in the data frame.")
  }
  # Retrieve the column names within the specified range
  quant_cols <- colnames(data)[RepRange]
  # QC check 1: ensure all column names contain "LFQ"
  if (!all(grepl("LFQ", quant_cols))) {
    # Identify column names missing "LFQ" to provide clearer error messages
    failed_cols <- quant_cols[!grepl("LFQ", quant_cols)]
    stop(paste("QC error: Some column names in the specified range do not contain 'LFQ'. Problematic columns:", 
               paste(failed_cols, collapse = ", ")))
  }
  # QC check 2: verify that the substring preceding "LFQ" is the same across columns
  # Use sub() to extract the prefix before "LFQ"
  prefixes <- sub("_LFQ.*", "", quant_cols)
  
  if (length(unique(prefixes)) > 1) {
    stop(paste("QC error: Sample prefixes (before 'LFQ') in the specified column range are inconsistent. Detected prefixes:",
...
for (df_idx in seq_along(ForStep19)) {
  ...
  if (method_name_trad %in% PARAM_MODELS_TO_PLOT) {
    ...
    for (group in names(group_info)) {
      tryCatch({
        # Use the actual column names defined in group_info, not hard-coded NES names
        ...
        if (!all(c(logfc_col, fdr_col) %in% colnames(original_data))) {
          cat(paste("    Warning: Experimental group", group, "is missing required logFC or FDR columns; skipping traditional filtering.\n"))
          next
        }
    ...
    for (control_name in names(spatial_control_groups)) {
      tryCatch({
        ...
        if (is.null(pattern) || pattern == "") {
          cat(paste0("    - Control group ", control_name, " did not provide a column pattern; skipping.\n"))
          next
        }
        ...
        if (length(control_cols) == 0) {
          cat(paste0("    - Control group ", control_name, ": no columns matched pattern '", pattern, "'.\n"))
          next
        }
        ...
        if (nrow(candidates_control) == 0) {
          cat(paste0("    - Control group ", control_name, ": matched columns exist but are all NA; skipping.\n"))
          next
        }
  ...
  cat("--- (C) Processing selection results for all AUC models...\n")
  ...
  if (length(all_summaries_list) > 0) {
    ...
    cat(paste("\n--- All results for data source '", data_source_name, "' have been aggregated.---\n", sep=""))
  }
...
# ===================================================================
#
### Section D: Visualization and file exports
#
# ===================================================================
# ===================================================================
#
### Section D: Visualization and file exports (revised)
#
# ===================================================================
...
for (data_source_name in names(detailed_candidate_list_all_sources)) {
  ...
  }, error = function(e){cat("Failed to save Excel file:", conditionMessage(e), "\n")})
}
...
for (data_source_name in names(final_plotting_data_list)) {
  ...
  # Prepare annotation data for diagrams
  label_df <- plot_df %>%
  ...
  # Draw count percentage charts
  p_count <- ggplot(plot_df, aes(x = Method, y = Percent, fill = !!sym(PARAM_GROUPING_COLUMN))) +
    geom_col(position = "stack") +
    # Fix: add x = Method inside aes()
    geom_text_repel(
      data = label_df, 
      aes(x = Method, label = label_text, y = 1.0), # <--- Key fix point
      inherit.aes = FALSE,
  ...
  cat(paste("Count percentage plot saved to:", output_pdf_count_path, "\n"))
  
  # Prepare annotations for abundance plots
  ...
  # Draw abundance percentage charts
  ...
  cat(paste("Abundance percentage plot saved to:", output_pdf_abundance_path, "\n"))
}
...
# ===================================================================
#
### Phase 3B: MultiBait_Localization stacked bars (Fig7 supplement) ####
#
# ===================================================================

cat("\n--- Starting MultiBait_Localization stacked bar chart (Fig7) ---\n")

# MultiBait colors and stacking order are configured by the MULTIBAIT_* interfaces at the top of the script
...
for (df_idx in seq_along(ForStep19)) {
  ...
  if ("Control" %in% PARAM_MODELS_TO_PLOT) {
    if (!"MultiBait_Localization" %in% colnames(original_data)) {
      cat("  - Warning: This data source is missing the MultiBait_Localization column; skipping control group handling.\n")
    } else {
      for (control_name in names(spatial_control_groups)) {
        tryCatch({
          control_cfg <- spatial_control_groups[[control_name]]
...
# Generate MultiBait_Localization stacked bars
...
for (data_source_name in names(final_plotting_data_multibait_list)) {
  ...
  # Set factor orders to control stacking
  ...
  # Prepare annotation data (show counts for the main SG-related categories)
  label_df <- plot_df %>%
  ...
  ggsave(output_pdf_count_path, p_count, width = 20, height = 12)
  cat(paste("MultiBait count percentage plot saved:", output_pdf_count_path, "\n"))
  ...
  ggsave(output_pdf_abundance_path, p_abundance, width = 20, height = 12)
  cat(paste("MultiBait abundance percentage plot saved:", output_pdf_abundance_path, "\n"))
}

cat("\n--- MultiBait_Localization stacked bar chart (Fig7) complete ---\n")


# ===================================================================
#
### Phase 3C: TP percentage curves (Fig9) ####
#
# ===================================================================

cat("\n--- Starting TP percentage curves (Fig9) ---\n")

# Create output directories
output_dir_phase3c <- file.path(output_dir_phase3, "TP_Percent_Curves")
dir.create(output_dir_phase3c, showWarnings = FALSE)

# Generate TP percentage curves for every data source (grouped by model to compare proximity labeling approaches)
for (df_idx in seq_along(ForStep19)) {
  original_data <- ForStep19[[df_idx]]
  data_source_name <- names(ForStep19)[df_idx]
  
  cat(paste("\nProcessing data source (TP percentage curve):", data_source_name, "\n"))
  
  # Stores data for all model × experimental group combinations
  all_model_data <- list()
  
  # Iterate over each experimental group to collect data
  for (group in names(group_info)) {
    cat(paste("  - Collecting experimental group data:", group, "\n"))
    
    # Retrieve the logFC column and sample columns relevant to the experimental group
    logfc_col <- group_info[[group]]$logFC
    fdr_col <- group_info[[group]]$logFC_FDR
    lfg_cols <- group_info[[group]]$samples
    
    # (A) TP percentage curve for traditional methods
    method_name_trad <- paste0("logFC>", PARAM_FC, " & FDR<", PARAM_FDR)
    if (method_name_trad %in% PARAM_MODELS_TO_PLOT) {
      tryCatch({
        if (all(c(logfc_col, fdr_col) %in% colnames(original_data))) {
          # Sort, label TP/FP, and exclude the SOD1 protein
          candidates_sorted <- original_data %>%
            filter(.data[[logfc_col]] > PARAM_FC & .data[[fdr_col]] < PARAM_FDR) %>%
            filter(rowSums(!is.na(dplyr::select(., one_of(intersect(lfg_cols, names(.)))))) >= PARAM_MIN_LFQ_NONNA_TPR) %>%
            filter(Gene != "SOD1") %>%  # Exclude the SOD1 protein
            arrange(desc(.data[[logfc_col]])) %>%  # Sort by logFC descending
            mutate(
              rank_pct = (row_number() / n()) * 100,
              is_TP = MultiBait_Localization %in% myTP_vector2
            )
          
          # Compute cumulative TP and FP lists
          ...
          if (nrow(candidates_trad_df) > 0) {
            ...
            # Store results in all_model_data keyed by model name
      ...
      for (model_name in names(res$candidates_full_list)) {
        if (model_name %in% PARAM_MODELS_TO_PLOT) {
          tryCatch({
            ...
            if (length(pass_genes) > 0) {
              # Gather full details, filter out SOD1, then match pass_genes
              full_candidates_info <- res$node_attributes %>% 
                filter(Gene != "SOD1") %>%  # First remove the SOD1 protein
                filter(Gene %in% pass_genes)
              
              # Choose the sorting column based on the model type
              ...
              # Compute the TP percentage curve
              # Sort first and label TP/FP
  ...
  # Generate plots for each model (comparing different proximity labeling approaches)
    cat(paste("\n  Generating TP percentage curves for all models...\n"))
  
  for (model_name in names(all_model_data)) {
    model_groups_data <- all_model_data[[model_name]]
    
    if (length(model_groups_data) > 0) {
      # Combine data from all experimental groups for that model
      model_combined <- bind_rows(model_groups_data)
      
      # Clean file names by removing Windows-disallowed characters < > : " / \ | ? *
      ...
      # Plot TP percentage curves (use different colors for each experimental group)
...
cat("\n--- TP percentage curves (Fig9) complete ---\n")


save.image(file = "Phase1toPhase3.RData")
print("Working environment saved successfully to Phase1toPhase3.RData")
